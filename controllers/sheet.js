...
exports.addrow = async(req, res) => {
    try {
            ...
            const addmyrow = main([new Date().toLocaleDateString().toString(), req.body.name, req.body.email, req.body.phone, req.body.company, req.body.message]);

        if (addmyrow) {
            return res.status(200).json("Row Added");
        }
    }catch (error) {
